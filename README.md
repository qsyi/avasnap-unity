...
リクエストが来るまでカメラの処理は行いません(`FileSystemWatcher`が主、環境によっては発火しないことがあるため0.5秒おきにリクエストファイルの更新日時だけを軽く確認するフォールバック付き)。Unityでカメラを動かしても自動では追従せず、押されるたびのスナップショット取得です。出力先は `%AppData%\AvaSnap\unity_camera_guide.json` です。
